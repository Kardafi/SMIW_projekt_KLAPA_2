...
BT_GATT_PRIMARY_SERVICE(BT_UUID_REMOTE_SERVICE),
    ...
    BT_GATT_CHARACTERISTIC(BT_UUID_REMOTE_WRITE_CHRC,
    ...
    BT_GATT_CHARACTERISTIC(BT_UUID_REMOTE_BATTERY_CHRC,
                    BT_GATT_CHRC_READ,
                    BT_GATT_PERM_READ,
                    read_battery_characteristic_cb, NULL, NULL),
);
...
void pwm_chrc_ccc_cfg_changed(const struct bt_gatt_attr *attr, uint16_t value)
{
    ...
    if (remote_service_callbacks.notif_changed) {
        remote_service_callbacks.notif_changed(notif_enabled?BT_PWM_NOTIFICATIONS_ENABLED:BT_PWM_NOTIFICATIONS_DISABLED);
    }
}
...
static ssize_t read_battery_characteristic_cb(struct bt_conn *conn, const struct bt_gatt_attr *attr,
			 void *buf, uint16_t len, uint16_t offset)
{
    read_battery(&battery_value);
    printk("Odczytano wartosc char. batrii:%"PRIu16"\n",battery_value);
...
void set_pwm_change(uint8_t _pwm_value)
{
